...
export default function Referral() {
  ...
  return (
    <div className="max-w-md mx-auto mt-6 mb-6 p-4">
      <h1 className="text-2xl font-bold text-white text-center mb-4">Refer Friends & Earn Rewards</h1>
      <div className="bg-gradient-to-r from-purple-500 to-blue-500 rounded-2xl p-6 shadow-xl relative overflow-hidden">
        <div className="absolute inset-0 flex justify-center items-center pointer-events-none">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="absolute w-2 h-2 bg-yellow-400 rounded-full animate-float"
              style={{ left: `${Math.random() * 100}%`, animationDelay: `${i * 0.5}s` }}
            />
          ))}
        </div>
        ...
        <div className="flex flex-col gap-4 z-10">
          ...
          <div className="flex items-center justify-between bg-white/10 rounded-lg p-3">
            <span className="text-white font-semibold">
              Your Referral Code: {user?.referralCode || 'REF123'}
            </span>
            <button
              onClick={handleCopy}
              className="bg-yellow-400 text-purple-900 px-3 py-1 rounded-lg font-bold hover:bg-yellow-300 transition-all duration-300 animate-bounce"
            >
              {copied ? 'Copied!' : 'Copy'}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              <p className="text-sm text-purple-100 font-semibold">Total Referrals</p>
              <p className={`text-2xl font-bold text-white ${isAnimatingReferrals ? 'animate-pulse' : ''}`}>
                {referrals.referralCount}
              </p>
            </div>
            <div className="text-center">
              <p className="text-sm text-purple-100 font-semibold">Referral Points</p>
              <p className={`text-2xl font-bold text-yellow-300 ${isAnimatingPoints ? 'animate-pulse' : ''}`}>
                {(referrals.referralPoints / 1000).toFixed(2)} $FLX
              </p>
            </div>
          </div>
      ...
      {referrals.referredUsers.length > 0 && (
        <div className="mt-6">
          <h2 className="text-xl font-bold text-white mb-2">Your Referrals</h2>
          <ul className="bg-white/10 rounded-lg p-4">
            {referrals.referredUsers.map((ref, index) => (
              <li key={index} className="text-white py-1">
                @{ref.username || `User${ref.telegramId}`}
              </li>
            ))}
          </ul>
        </div>
      ...
      {error && (
        <div className="absolute top-0 left-0 right-0 bg-red-500/80 text-white text-center py-2">
          {error}
        </div>
      )}
